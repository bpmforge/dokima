 ...
 * served by the real apps/server against a throwaway `.shipwright` home
...
export async function startApp(port) {
  ensureSpaBuilt();
  const home = mkdtempSync(path.join(os.tmpdir(), 'shipwright-capture-home-'));
  const projectDir = mkdtempSync(path.join(os.tmpdir(), 'shipwright-capture-project-'));
  ...
  const server = spawn(TSX_BIN, ['src/api/main.ts'], {
    ...
    env: {
      ...
      SHIPWRIGHT_STATE_DB: path.join(home, 'capture-state.db'),
    },
    stdio: 'ignore',
  });
