 ...
 *
 * HISTORY NOTE (W16-07): this header used to carry a long "KNOWN GAP"
 * describing validate-mermaid emitting zero-byte stdout on a clean scan,
 * which made every phase's gate un-mintable against real content. W9-08
 * fixed the script on 2026-07-28 (content/validators/validate-mermaid.sh
 * now sources _lib.sh and always emits a conforming envelope), so a clean
 * gate receipt IS mintable on real, unmodified content — proven end to end
 * by the gate->advance RED fixture in ../pipeline-routes/advance.test.ts.
 */
