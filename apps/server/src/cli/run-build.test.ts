...
describe(
  'executeBuildRun (W11-04, FR-H6, D-023)',
  () => {
    ...
    it('W12-02: the encrypted-file fallback is a working store, not a refusal — the supported non-macOS path still runs', async () => {
      ...
      project = await gitRepoProject();
      const log = openWritableLog(resolveDbPath(project.cwd));
      // W14-06: post-run consolidation appends as the run actor. On macOS this
      // test never got that far (the patched platform makes the sandbox probe
      // refuse first); on a Linux runner the run completes and the append's
      // actor FK is real (P6-21).
      createIdentity(log, { id: 'worker-1', name: 'worker-1', kind: 'machine' });
      const realPlatform = process.platform;
    ...
    it(
      ...
      () => {
        expect(tokenizeAgentCommand('"/Applications/My Agent/bin/agent" --flag')).toEqual(
          ['/Applications/My Agent/bin/agent', '--flag'],
        );
...
describe(
  'executeBuildRun policy wiring (W12-18)',
  () => {
    let project: TempProject;
    afterEach(async () => {
      await project?.cleanup();
  ...
  () => {
    ...
    it('a local provider kind earns the 12-attempt ceiling, inferred not asked (D-027)', () => {
      for (const providerKind of ['lm-studio', 'ollama']) {
        const result = resolvePolicyScope(
          { mode: 'pinned', model: 'qwen', providerKind },
          'coding-agent',
        );
        expect('refusal' in result).toBe(false);
        if ('refusal' in result) return;
        expect(result.scope.global?.['coding-agent']).toMatchObject({
          tierKind: 'local',
        });
      }
    });
 ...
 */
describe(
  'resolveMaxToolIterations (W13-11)',
  () => {
    ...
    it('refuses a nonsense value by name instead of coercing it', () => {
      for (const bad of ['20', 0, -3, 2.5]) {
        ...
        expect('refusal' in result, `${JSON.stringify(bad)} should be refused`).toBe(
          true,
        );
      }
    });
 ...
 */
describe(
  'the external agent runs under a watchdog (W13-47)',
  () => {
...
describe('the session runs as the machine, not as whoever launched it (W21-70, C-4)', () => {
  ...
  it(
    'the machine identity is kind "machine", so it cannot sign a human-only receipt',
    async () => {
  ...
  it(
    'the run bookkeeping still records the human who launched it',
    async () => {
