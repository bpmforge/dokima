...
describe('buildSandboxEnv', () => {
  const originalSecret = process.env.SHIPWRIGHT_TEST_SECRET;

  afterEach(() => {
    if (originalSecret === undefined) delete process.env.SHIPWRIGHT_TEST_SECRET;
    else process.env.SHIPWRIGHT_TEST_SECRET = originalSecret;
  });

  it('never carries an arbitrary ambient var through, even one that looks like a credential', () => {
    process.env.SHIPWRIGHT_TEST_SECRET = 'sk-should-never-leak';
    const env = buildSandboxEnv();
    expect(env.SHIPWRIGHT_TEST_SECRET).toBeUndefined();
  });
  ...
  it('carries through only the safe allowlist (PATH survives, HOME survives)', () => {
    ...
    expect(Object.keys(env).every((key) => key !== 'SHIPWRIGHT_TEST_SECRET')).toBe(true);
  });
