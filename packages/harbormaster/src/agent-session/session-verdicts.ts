...
export async function refuseIfSessionExceededScope(
   ...
   * are written by the harness itself — the dependency install's lockfile and
   * the .gitignore line that keeps node_modules out of this very diff.
   ...
   *
   * W23-23: the SAME function the close gate and the review-evidence check
   * use, not a copy of its filter — validator telemetry moved under .dokima/
   * and this copy would have kept exempting a path the harness no longer
   * writes.
   */
  const agentPaths = agentAuthoredPaths(changedPaths);
