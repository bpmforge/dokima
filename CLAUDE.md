...
1. **One ticket at a time** from `plan.json`; respect `write_scope`, `lane`,
   ...
   **widen**, when the file is unowned and this ticket's own acceptance needs
   ...
   _a follow-up that names no ticket id is not a deferral, it is a dropped
   finding._ `validate-plan`'s P13 reports the ones that slip.
...
--filter @dokima/web e2e`**, and **`pnpm validate`** (the six repo
   ...
   in commit bodies.
   _`pnpm validate` joined the gate 2026-08-28. The six validators — plan,
   ...
   into a check nobody can trust._
   _temp-leaks runs beside them, not among them (W22-18, 2026-08-29). Three consecutive tickets
   ...
   source, this one asks about the machine after a run._
   _The six run CONCURRENTLY as of W23-15 (`scripts/gate-plan.mjs`): same
   validators, same ratchet arguments read from the same `conductor.config.json`,
   same nonzero-if-any-fails rule, ~11s → ~6.4s. They are printed in the
   configured order whatever order they finish in, so "the first failure you
   read is the first one in the list" is still true — that was always a
...
9. **Two rules that were one, and must not be re-merged** (C-1, D-024):
   ...
   (b) **Product guarantee, not a default:** a user who _chooses_ local-only
