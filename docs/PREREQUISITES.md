# Dokima — Human prerequisites (the ONLY human steps, per wave)
...
| HP-5 | before W6 forge connect            | Create the two machine accounts (`dokima-maker`, `dokima-reviewer`) on the forge; generate separately-scoped tokens (maker: push/PR/comment; reviewer: review/accept only) | W6-03, SC-03, C-4                    |
| HP-6 | before W6 forge connect            | Confirm you hold admin on the target repo (branch-protection setup needs it, SC-14)                                                                                        | W6-01                                |
| HP-7 | pre-0.3 public tag                 | Founder decisions: LICENSE choice (Apache-2.0 vs MIT, D-006) + naming pass (shipwright.io collision, D-001/S-43 — done via D-021 rename)                                   | RELEASE_TRACKER pre-public checklist |
