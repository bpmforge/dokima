...
| **W12** | Maintenance, unfiled-finding intake, and the context layer | W12-01…07 | 27 | Opened 2026-08-13. **No pre-planned exit criteria, and that is deliberate rather than an omission** — this wave is an intake lane for defects found outside a planned wave, written the way W9's row was after the fact. Opened with W12-01 (a `DEFAULT_PORT` constant declared twice, so changing the port is a two-file edit no gate can see) and W12-02/03, the two `medium[]` findings from `docs/work/SECURITY_W11f.md` that were never carried onto the board when that pass's CRITICAL and HIGH became W11-20. **The intake gap was the wave's original subject:** per-wave security docs are written once and nothing re-reads them, so a finding below the fold expires quietly. **Then the wave changed character on the same day it opened.** A step-back review asking whether Dokima implements its own product thesis — *the product manages the context* — found that it does not, on the build path: `defaultHandoffBuilder` sets `context: ticket.interface ?? ticket.title`, and every ticket session the built-in agent has ever run received one line of context. FR-L5's Context Packer is fully built and tested in `packages/memory/src/packer/` and had **zero callers**, because `packages/memory/src/index.ts` exports only `lessons/*` and the packer is unreachable across the package boundary. The same review found `anchors.ts` (FR-L2) firing nowhere and the build loop dispatching exactly one of 93 experts. Filed as **W12-04** (wire the packer, 8), **W12-05** (wire the tool anchor, 5), **W12-06** (role selection — **HELD**, founder call on routing policy, 5) and **W12-07** (attest content re-import 3.1.24 → 3.5.1, 3). **The pattern under all four is one this board has now hit five times** (W11-04's `RunSessionInput.secretValues`, W11-16's wrap-`spawn`, and now the packer, the anchors and the roster): a mechanism lands complete, with a documented contract and no production caller, and stays green forever because **no gate can see an interface with no callers**. That is the wave's real subject now, and it is a bigger one than the intake gap. W12 closes when its tickets close. |
