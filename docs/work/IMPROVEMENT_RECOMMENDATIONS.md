# Dokima — P2 Domain Interrogations → Recommendation Index

**Date:** 2026-07-14 · **Branch:** `review/design-review-hardening` · companion to `docs/work/DESIGN_REVIEW.md` (P1 gap register)
> **ADOPTED 2026-07-14 — "adopt all" (Brad).** AM-1→**D-015** (FR-T3 territory-release) · AM-2→**W3-13** (secrets pull-forward, was W8-03) · AM-3→**D-014** (rules-first gate economics, FR-RL1–4) · AM-4→**D-016** (Improvement Plans, FR-PLAN1–4, W5-10/11) · AM-5 tier-aware ceiling **confirmed** · AM-6→BLUEPRINT §3.5 micro-cap-2 + §3.2 asymmetric scoring + FR-L1/L6 · AM-7→**D-017** (Apache-2.0, flagged vetoable).
> Threaded through: DECISIONS D-014..D-017 · BLUEPRINT §3.2/§3.4/§3.5/§6.1 (FR-RL, FR-PLAN, FR-T3, FR-L1 wording) · SRS §2.3 FR-T3, §2.5 FR-L1/L6, new §2.13 FR-RL + §2.14 FR-PLAN · SCOPE S-30/S-31 + S-28 note · DATABASE §5b (rule_state, findings, suppressions, plan_items) · USER_STORIES US-108/109/309/310/408/409/410 · ROADMAP (301 pts / 75 tickets; W3+W5 exit criteria) · plan.json (W3-13 moved, W5-10/11 added, W3-08 widened to 8 pts, amendments to W4-06/07/08, W5-06/08, W6-07, W7-04, W8-01).
> R-B3/R-B4/R-F5 are doc-notes/resume-prep (done in place); R-E2/R-F3 land in P4; R-F4 lands in the P5 W3-01 split. NOT adopted: nothing — see the standing "Rejected during interrogation" list at the bottom for what was pre-rejected with reasons.
> **Follow-up interrogation opened same day** (Brad's adopt-all message introduced six new domains): see §G–§L addendum below when it lands, tracked in this file.

Sections mirror the interrogation domains: **A** process/UX · **B** micro-agent & loop architecture · **C** reports→action · **D** false-positive economics · **E** code-health content & extensibility · **F** core-brain/trust. Each rec is `**R-X-n (effort S/M/L, wave)**`. Evidence basis: Dokima's own conductor telemetry (CONDUCTOR_FIELD_REPORT.md — 70 sessions, 7 harness failures all mapping to source-system mechanisms, 75% pre-fix false-block rate), the bpm-opencode-experts protocol suite (read at v2.9.0, 2026-07-14), and RepoPulse's shipped designs (IMPROVEMENT_PLANS.md, SECURITY_SUITE §3.6 VEX, CODE_HEALTH_SUITE §8). External web research deliberately skipped: every mechanism recommended here has already run in one of the three internal systems — primary evidence beats competitor blog posts (kickoff §5 would mark them UNVERIFIED anyway).
...
**R-G3 (S, doc note)** — **Session/model receipts parity confirmed**: the library's `session-receipts.jsonl` + escalation-ledger Hop records are already covered by Dokima's `model.call_completed` events + budget_ledger (richer). The library's "telemetry-driven tier routing" is *not built there either* — noted as a shared post-v1 horizon, not a gap.
**R-G4 (S, backlog note)** — Jira adapter (v2.6/2.7) and Figma adapter (v2.9) exist upstream; Dokima's forge-adapter surface (FR-I2 plug-in API) is the natural home **post-v1** — recorded in ROADMAP post-1.0 horizon, not ticketed.
