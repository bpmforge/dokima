...
Blueprint: an idea and no team; Dokima is the PM, architect, security reviewer,
and dev crew.
...
- **Berths:** the power user of parallelism — berths=3+ across lanes, breakpoint
  ...
  (`dokima run --breakpoint wave`) since UI and CLI drive the same verbs.
...
- **Willingness to pay (hypothesis):** already pays for one or two AI dev
  subscriptions; will pay professional-tool rates if Dokima demonstrably
...
- **Morning queue:** triage surface for the team's overnight agent shift; sorted-by-
  ...
  their normal forge review flow — Dokima does not ingest forge comments in v1
...
- v1 single-operator (D-005) means P3's teammates are *readers* of mirrored forge
  state, not Dokima users; do not design v1 UI affordances that pretend
  otherwise.
