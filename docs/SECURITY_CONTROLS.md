# Shipwright — Security Controls Catalog
...
- **SC-01 Write-scope enforcement via git diff** (T-3, T-4, T-11, T-23). After every agent
  ...
  (`.github/workflows/**` etc. — grantable only via explicit human approval), `.shipwright/**`,
...
- **SC-03 Per-identity forge tokens; reviewer isolation** (T-9, T-10). Forge connect
  provisions two machine identities with separately scoped tokens: `shipwright-maker`
  (push branches, open PRs, comment) and `shipwright-reviewer` (review/accept only). The
...
- **SC-11 Audit hash chain + verification command** (T-8, T-23, T-24, T-25). Events are
  hash-chained per DATABASE.md §2 with INSERT-only triggers. `shipwright audit verify`
  ...
  High-water seq mirrored to `~/.shipwright/` per project; when the forge mirror is
...
- **SC-16 Supply-chain policy for Shipwright itself** (T-22). pnpm lockfile +
