...
- **SC-01 Write-scope enforcement via git diff** (T-3, T-4, T-11, T-23). After every agent
  ...
  matching; new symlinks pointing outside ⇒ refuse). _Lands:_ W0 · packages/git +
  harbormaster. _Verify:_ fixture sessions with `../` paths, symlink escapes, hook edits,
...
- **SC-02 Out-of-session gate re-execution** (T-1, T-5, T-17). No state derived from
  ...
  currency, FR-P2). _Lands:_ W0 (receipt primitive) / W3 (Harbormaster loop).
  _Verify:_ conformance suite ports the source systems' gate-integrity fixtures (D-008):
...
- **SC-03 Per-identity forge tokens; reviewer isolation** (T-9, T-10). Forge connect
  ...
  token — all forge writes go through the core. _Lands:_ W6 · packages/forge.
  _Verify:_ env snapshot test on spawned sessions (zero token vars); planted-token
...
- **SC-04 Promise-token ban — receipt-based completion only** (T-1, T-2). No code path
  ...
  loop/harbormaster/pipeline. _Lands:_ W0 rule, permanent. _Verify:_ lint gate in CI +
...
- **SC-05 Waiver & approval human-signature enforcement** (T-5, T-24). Waiver receipts and
  ...
  morning queue (FR-N3). _Lands:_ W0 · packages/events. _Verify:_ unit tests minting
...
- **SC-06 Secrets vault + universal redaction** (T-6, T-9, T-16, T-20). Provider and forge
  ...
  close (BLUEPRINT §12 item 5). _Lands:_ **W3-13 per plan.json** (design-review 2026-07-14:
  ...
  the autonomous W3 runs; the W8-01 dogfood re-verifies end-to-end). _Verify:_ planted
...
- **SC-09 Signed content packs** (T-21, D-006). Expert/validator packs carry a manifest
  ...
  never `eval`s pack markdown. _Lands:_ **W6-07 per plan.json** (design-review 2026-07-14
  ...
  install flow). _Verify:_ tampered-pack fixture ⇒ refused; unsigned path shows badge;
...
- **SC-08 Localhost binding + origin allowlist + auth token** (T-19, T-20, D-005).
  ...
  middleware: bearer token (128-bit, generated first-run, `~/.dokima/token`, mode 0600) + `Host`/`Origin` allowlist (`localhost:<port>` exact) — defeats DNS rebinding and
  cross-site localhost CSRF; no CORS headers served at all. WS upgrades re-check token +
  Origin. `~/.dokima/` and `.dokima/` are created 0700. _Lands:_ **W4-01 per
  ...
  schema level only, identities table W0-02). _Verify:_ route-walker (API_DESIGN §4); rebinding simulation (evil Host header ⇒ 403);
...
- **SC-10 NEVER-AUTO single enforcement point** (T-4, T-11, T-14). The immutable list
  ...
  _Lands:_ W3 · harbormaster. _Verify:_ per-class tests in `auto` mode with breakpoint
...
- **SC-11 Audit hash chain + verification command** (T-8, T-23, T-24, T-25). Events are
  ...
  anchor no local writer can rewrite (closes T-25). _Lands:_ W0 · packages/events.
  _Verify:_ tamper fixtures (row edit, row delete, truncation) each detected; e2e:
...
- **SC-12 MCP permission matrix** (T-13, T-14, T-15). Agents request; the core executes.
  ...
  over-broad scopes (filesystem root). _Lands:_ W6 · packages/mcp. _Verify:_ allowlist
...
- **SC-13 Watchdog, session caps, budget breakers** (T-7, T-18). Per-session wall-clock
  ...
  70/85/100% aggregate across berths, hard stop at ticket boundary. _Lands:_ W2 (breakers)
  / W3 (watchdog). _Verify:_ stall fixture killed + dead-lettered; breaker fixtures at
...
- **SC-14 Branch protection on forge connect** (T-10, T-11). Connect flow configures:
  ...
  parity validator re-reads protection settings and flags drift. _Lands:_ W6 ·
  packages/forge. _Verify:_ integration test against a scratch repo asserts settings;
...
- **SC-15 Mirror reconciliation audit** (T-12). Lifecycle verbs write through to the
  ...
  forge-not-local) surfaced as a Review notification (FR-T5). _Lands:_ W6. _Verify:_
...
- **SC-16 Supply-chain policy for Dokima itself** (T-22). pnpm lockfile +
  ...
  verified against registry + docs before import (anti-slopsquatting). _Lands:_ W0 · repo
  root + CI. _Verify:_ CI asserts `.npmrc` + audit green.
...
- **SC-17 Tool-boundary write-scope enforcement** (T-3, T-4, T-11, T-23, T-24; D-023).
  ...
  **SC-01 STILL RUNS AFTERWARDS AND IS STILL AUTHORITATIVE — by TWO independent
  ...
  _Lands:_ the agent-session wave · `packages/harbormaster/src/agent-session/**` (W11-03;
  corrects an earlier `apps/server` placement recorded before the challenge pass moved
  this scope to harbormaster alongside D-023's session loop). _Verify:_ red fixtures for a
...
- **SC-18 Tool surface is closed and least-privilege** (T-5, T-24; D-023). The session's
  ...
  tool SET. _Verify:_ a session whose model emits a call to an unlisted tool gets a refusal
...
/api/v1/browse` exists because `onboard` and `import` need a directory only the
  ...
  whole filesystem. Directories only, dot-directories omitted. _Verify:_ a
...
- **SC-20 The receipt signing key is minted, never requested, and never
  ...
  _unreadable_ store (wrong `DOKIMA_VAULT_KEY`, unlockable keychain entry) is
  a third, separately named refusal, never treated as "no key yet". _Verify:_
