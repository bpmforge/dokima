...
git(target, ['add', '-A']);
git(target, ['commit', '-m', 'chore: three-ticket scratch project']);
...
try {
  ...
  createTicket(log, 'worker-1', {
    id: 'T-3', type: 'task', title: 'Satisfy a gate that cannot be satisfied',
    lane: 'docs', interface: 'n/a', writeScope: ['docs/**'], verify: 'node src/check3.mjs',
    acceptance: [{ id: 'AC-1', text: 'The verify command exits 0 (it never does).', done: false }],
  });
} finally {
  log.close();
}
console.log('three-ticket scratch ready:', target);
